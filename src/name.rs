...
impl DnsNameInput for String {
    type Storage = Box<[u8]>;
    fn into_storage(self) -> Self::Storage { self.into_boxed_str().into() }
}

#[cfg(feature = "std")]
impl DnsNameInput for Box<[u8]> {
    type Storage = Box<[u8]>;
    fn into_storage(self) -> Self::Storage { self }
}

#[cfg(feature = "std")]
impl DnsNameInput for Vec<u8> {
    type Storage = Box<[u8]>;
    fn into_storage(self) -> Self::Storage { self.into() }
}

impl<'a> DnsNameInput for &'a str {
    type Storage = &'a [u8];
    fn into_storage(self) -> Self::Storage { self.as_ref() }
}
    ...
    B: AsRef<[u8]>,
{
    ...
    pub fn try_from_punycode(input: impl DnsNameInput<Storage = B>) -> Result<Self, InvalidDnsNameError>
    {
...
pub(crate) fn verify_cert_dns_name(
    cert: &super::EndEntityCert, DnsName(dns_name): DnsName<&[u8]>,
) -> Result<(), Error> {
    let cert = &cert.inner;
